...
st.set_page_config(
    page_title="Cat Emotion Detector (Video/Image)",
    page_icon="📸",
    layout="wide",
...
def main():
    st.markdown('<p class="main-header">🎬 Cat Emotion Detector (Video & Image)</p>', unsafe_allow_html=True)
    st.markdown('<p class="sub-header">Frame-by-Frame / Image Analysis Powered by Deep Learning</p>', unsafe_allow_html=True)
    ...
    if analysis_type == "Image 🖼️":
        st.subheader("📤 Upload Cat Image")
    ...
    if uploaded_file is not None:
        ...
        if file_type == "image" and analysis_type == "Image 🖼️":
            st.markdown("### 🖼️ Input Image")
            image = Image.open(uploaded_file).convert("RGB")
            st.image(image, caption="Uploaded Image", use_column_width=True)
            st.markdown("---")

            if st.button("🔬 Analyze Cat Emotion", type="primary", use_container_width=True, key="analyze_image"):
                if st.session_state.model:
                    with st.spinner("Analyzing image..."):
                        try:
                            frame_bgr = cv2.cvtColor(np.array(image), cv2.COLOR_RGB2BGR)
                            predicted_emotion, confidence = predict_frame_emotion(st.session_state.model, frame_bgr)
        ...
        elif file_type == "video" and analysis_type == "Video 🎬":
            ...
            if st.button("🔮 Analyze Cat Emotions", type="primary", use_container_width=True, key="analyze_video"):
                if st.session_state.model:
                    with st.spinner("Analyzing video... This might take a while..."):
                        try:
                            ...
                            if output_video_path and emotions_summary_counter:
                                ...
                                if total_frames > 0: df_summary['Percentage'] = (df_summary['Frame Count'] / total_frames)
                                else: df_summary['Percentage'] = 0.0
                                ...
                                else: st.warning("No emotions detected.")
                            else: st.error("❌ Video processing failed.")
                        except Exception as e: st.error(f"❌ Error during video analysis: {str(e)}")
                else: st.warning("⚠ Model not loaded!")

    if st.session_state.prediction_history:
        st.markdown("---")
        st.subheader("📜 Analysis History (Videos Only)")
