...
# Import streamlit with a safe fallback for non-Streamlit contexts
try:
    import streamlit as st
except ImportError:
    # Create a dummy st module when streamlit is not available
    class _DummySt:
...
def preprocess_frame(frame):
    try:
        ...
        if frame is None or frame.size == 0:
            raise ValueError("Empty or None frame provided")
...
def predict_frame_emotion(model, frame):
    try:
        # Validate frame is not None and has proper shape
        if frame is None:
            print("Error: Frame is None")
            return "N/A", 0.0
        
        if len(frame.shape) != 3:
            print(f"Error: Frame has invalid shape {frame.shape}, expected (H, W, C)")
            return "N/A", 0.0
        ...
        # Debug logging
        print(f"Frame prediction: emotion={predicted_emotion}, confidence={confidence:.4f}, class_idx={predicted_class_index}")
...
def process_video_frames(video_path, model):
    ...
    # Create a progress bar if Streamlit context exists; otherwise use a no-op fallback
    try:
